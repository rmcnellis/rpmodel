...
lue_vcmax_c4 <- function( out_optchi, kphio, c_molmass, soilmstress ){
  ...
  ## factors derived as in Smith et al., 2019
  omega <- omega( theta = theta, c_cost = 0.01, m = out_optchi$mj )
  omega_star <- 1.0 + omega - sqrt( (1.0 + omega)^2 - (4.0 * theta * omega) )
...
calc_kp <- function(tc, elv){ # Boyd 2015
  patm = patm(elv)
  rat = patm / patm(0)
...
chi_c4 <- function( tc, elv, beta, ns_star, vpd, gammastar, ca, kmm, oa, c_cost, kphio, ppfd ){
  ...
  theta <- 0.85
  oi <- chi * oa
  omega <- omega( theta = theta, c_cost = 0.01, m = mj )
  omega_star <- 1.0 + omega - sqrt( (1.0 + omega)^2 - (4.0 * theta * omega) )
  elv_km <- elv / 1000 
  par <- ppfd * (1 + 0.027 * elv_km)
  Al <- kphio * par * mj * omega_star / (8 * theta)
  leakiness <- 0.01
  leakage <- leakiness * Al
  cbs <- (leakage + (0.003 * ci)) / 0.003
  kc <- calc_kc(tc, elv)
  ko <- calc_ko(tc, elv)
  
  mc <- ((cbs + kc * (1 + oi/ko)) / (cbs - gammastar))

  ## mj:mv
  mjoc <- (chi + kappa) / (chi + 2 * gamma)  
  
  out <- list(xi = xi, chi = chi, mc = mc, mj = mj, mjoc)
...
calc_lma <- function(deciduous = 'yes', # yes or no for deciduous species
                     ...
                     ppfd, # light absorbed in µmol m-2 s-1
                     tc, # temperature in C
  ...
  # calculate M-M terms
  K <- calc_k( tc, pressure )
  f1 <- exp( -0.0227 * ( tc - 25 )) # the viscosity of water relative to its value at 25˚C
  ...
  # calculate hT
  dS.v <- 668.39 - 1.07 * ( tc + 273.15 )
  hT <- ( exp ( Ha.v * ( tc - 25 ) / ( 298.15 * R * ( tc + 273.15 ))) ) * 
    ( ( 1 + exp(( 298.15 * dS.v - 200000) / ( 298.15 * R ))) / 
        ( 1 + exp((( tc + 273.15) * dS.v - 200000) / (( tc + 273.15 ) * R ))))
  
  if( deciduous == 'yes' ){
    # deciduous LMA
    xTde <- hT * mc / ( kphio * m )
    Cde <- log( k1 ) + log( 365 ) - log( u ) - log( xTde )
    LMA <- exp( log( f ) + log( ppfd ) - 0.052 * tc + Cde )
  }
  
  else{
    # evergreen LMA
    LMA <- exp( 0.5 * log( ppfd ) - 0.013 * temperature - 0.12 * log( alpha ) + 0.25 * log( f ) + 3.5 )
  }
