...
rpmodel <- function(
  ...
  ca <- co2 * 1e-6 * patm
  ## ambient O2 partial pression (Pa)
  oa <- o2 * 1e-6 * patm
  
  ## photorespiratory compensation point - Gamma-star (Pa)
  if (c4){
    gammastar <- gammastar_c4( tc, elv )
  ...
  ns      <- viscosity_h2o( tc, elv )  # Pa s
  ns25    <- viscosity_h2o( kTo, elv )  # Pa s
  ...
  if (c4){
    ...
    out_optchi <- chi_c4( tc, elv, beta, ns_star, vpd, gammastar, ca, kmm, oa, c_cost, kphio, ppfd )
  ...
  if (c4){
    ...
    vpmax <- ((kp + ci) / ci) * 
      (kphio * iabs * out_optchi$mj * out_lue_vcmax$omega_star / 
         (8 * theta))
  ...
  vpmax25 <- vpmax / calc_tresp_mult(tc, tc, 25)
  ...
  if (c4){
    ...
    Al <- kphio * par * out_optchi$mj * out_lue_vcmax$omega_star / (8 * theta)
    
    leakiness <- 0.01
    leakage <- leakiness * Al
    cbs <- (leakage + (0.003* ci)) / 0.003
    
    oi <- oa * out_optchi$chi
    
    kc <- calc_kc(tc, elv)
    ko <- calc_ko(tc, elv)
    
    vcmax <- (kphio * par * out_optchi$mj *
                out_lue_vcmax$omega_star / 
                (8 * theta)) * ((cbs + kc * (1 + oi / ko)) / (cbs - gammastar))
    a_c <- vcmax * (1 / out_optchi$mc)
  ...
  # calculate leaf mass per area
  lma <- ifelse(do_lma, 
                calc_lma(deciduous = deciduous, f = f, ppfd = ppfd, tc = tc, vpd = vpd, elv = elv, ca = ca,
                         kphio = kphio, alpha = alpha, beta = beta), 
                lma)
  ...
  out <- data.frame(
    ...
    "gammastar"       = gammastar,
    "ns_star"         = ns_star,
    "omega"           = out_lue_vcmax$omega,
    "omega_star"      = out_lue_vcmax$omega_star,
    "kmm"             = kmm,
    "ns_star"         = ns_star,
    "cbs"             = cbs,
    "Al"              = Al,
